...
your_domain = "barosulbe.com"
sites = [{"name": "yoksil"}, {"name": "sujun"}, {"name": "bungi"}]
categories = [
    {"category": "변기"},
    {"category": "세면대"},
    {"category": "수전"},
...
if os.path.exists('dong3.txt'):
    with open('dong3.txt', 'r', encoding='utf-8') as f:
        regions = [line.strip() for line in f if line.strip()]
else:
    regions = ['인천 광역시']

all_images = [f"/images/{i}.png" for i in range(1, 7)]
work_images = [f"/images/work_sulbi/{i}.jpg" for i in range(1, 31)]
...
def get_service_description_sample(category_service):
    for category, description in service_descriptions.items():
        if category in category_service:
            return random.sample(description, 4)
    return ""
...
''')
    ...
    for i in range(1, 7):
        for reg in regions:
            ...
            sink_desc = get_service_description_sample("싱크대")[0] if "싱크대" in cat else ""
            sink_desc2 = get_service_description_sample("싱크대")[1] if "싱크대" in cat else ""
            sink_desc3 = get_service_description_sample("싱크대")[2] if "싱크대" in cat else ""
            sink_desc4 = get_service_description_sample("싱크대")[3] if "싱크대" in cat else ""
            sujun_desc = get_service_description_sample("수전")[0] if "수전" in cat else ""
            sujun_desc2 = get_service_description_sample("수전")[1] if "수전" in cat else ""
            sujun_desc3 = get_service_description_sample("수전")[2] if "수전" in cat else ""
            sujun_desc4 = get_service_description_sample("수전")[3] if "수전" in cat else ""
            byeongi_desc = get_service_description_sample("변기")[0] if "변기" in cat else ""
            byeongi_desc2 = get_service_description_sample("변기")[1] if "변기" in cat else ""
            byeongi_desc3 = get_service_description_sample("변기")[2] if "변기" in cat else ""
            byeongi_desc4 = get_service_description_sample("변기")[3] if "변기" in cat else ""
            semyondae_desc = get_service_description_sample("세면대")[0] if "세면대" in cat else ""
            semyondae_desc2 = get_service_description_sample("세면대")[1] if "세면대" in cat else ""
            semyondae_desc3 = get_service_description_sample("세면대")[2] if "세면대" in cat else ""
            semyondae_desc4 = get_service_description_sample("세면대")[3] if "세면대" in cat else ""
            baegwan_desc = get_service_description_sample("배관")[0] if "배관" in cat else ""
            baegwan_desc2 = get_service_description_sample("배관")[1] if "배관" in cat else ""
            baegwan_desc3 = get_service_description_sample("배관")[2] if "배관" in cat else ""
            baegwan_desc4 = get_service_description_sample("배관")[3] if "배관" in cat else ""
            hasu_desc = get_service_description_sample("하수구")[0] if "하수구" in cat else ""
            hasu_desc2 = get_service_description_sample("하수구")[1] if "하수구" in cat else ""
            hasu_desc3 = get_service_description_sample("하수구")[2] if "하수구" in cat else ""
            hasu_desc4 = get_service_description_sample("하수구")[3] if "하수구" in cat else ""
            hwajang_desc = get_service_description_sample("화장실")[0] if "화장실" in cat else ""
            hwajang_desc2 = get_service_description_sample("화장실")[1] if "화장실" in cat else ""
            hwajang_desc3 = get_service_description_sample("화장실")[2] if "화장실" in cat else ""
            hwajang_desc4 = get_service_description_sample("화장실")[3] if "화장실" in cat else ""
